  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Form Schemas</h2>
          <p className="text-gray-500 text-sm mt-1">Customize fields for donation, registration, expense, and other forms</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {FORM_TYPES.map(ft => {
          ...
          return (
            <div key={ft} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b">
                <h4 className="font-semibold text-gray-700 text-sm">{FORM_TYPE_LABELS[ft]}</h4>
                <p className="text-xs text-gray-400">{typeSchemas.length} schema(s) — only one can be active</p>
              </div>
              {typeSchemas.length === 0 ? (
                <div className="px-4 py-5 text-sm text-gray-400 text-center">No schemas — click "+ New Schema" to create one</div>
              ) : (
                <table className="min-w-full divide-y divide-gray-100">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {typeSchemas.map(s => (
                      <tr key={s.schema_id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm text-gray-600">#{s.schema_id}</td>
                        <td className="px-4 py-3">
                          <button onClick={() => toggleActive(s)}
                            className={`text-xs font-semibold px-2 py-1 rounded-full ${s.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
        ...
        {loading && (
          <div className="text-center py-8 text-gray-400">Loading schemas…</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-gray-800">{isCreating ? 'Create Form Schema' : `Edit Schema #${selected?.schema_id}`}</h3>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <div className="mb-4">
                <label className="flex items-center gap-2 cursor-pointer mb-4">
                  ...
                  <span className="text-sm font-medium text-gray-700">Set as Active schema for {FORM_TYPE_LABELS[form.form_type]}</span>
                </label>
              </div>

              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">Schema JSON</label>
                <p className="text-xs text-gray-500 mb-2">Array of field definitions. Each field: name, label, type (text/number/email/textarea/select/checkbox/date), required, placeholder, options (for select).</p>
                <textarea rows={22} value={form.schema_json}
                  onChange={e => { setForm(f => ({ ...f, schema_json: e.target.value })); validateJson(e.target.value); }}
                  className={`w-full border rounded-lg px-3 py-2 text-xs font-mono focus:ring-2 focus:ring-blue-500 ${jsonError ? 'border-red-400' : 'border-gray-300'}`} />
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t bg-gray-50">
              ...
              <button onClick={save} disabled={saving || !!jsonError} className="px-5 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium disabled:opacity-50">

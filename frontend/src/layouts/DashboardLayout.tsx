...
export default function DashboardLayout({ children, menuItems, title, role }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex bg-background-light">
      ...
      {/* Main Content — no overflow-hidden so fixed/sticky children render freely */}
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3 min-w-0">
            <button
              className="lg:hidden p-2 -ml-1 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-xl focus:outline-none transition-colors shrink-0"
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                title="My account"
              >
                <div className="h-8 w-8 rounded-full bg-primary-600 text-white flex items-center justify-center font-semibold text-sm shrink-0">
                  {initials}
                </div>
              ...
              {/* Dropdown menu — fixed on mobile so it always shows above content */}
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-[100]">
                  {/* User info header */}
